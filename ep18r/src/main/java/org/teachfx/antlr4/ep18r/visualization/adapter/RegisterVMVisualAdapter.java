package org.teachfx.antlr4.ep18r.visualization.adapter;
 ...
 * EP18R寄存器虚拟机可视化适配器
 * 
 * <p>实现IVirtualMachineVisualization接口，为EP18R寄存器虚拟机提供统一的可视化支持。
 * 该适配器包装RegisterVMInterpreter实例，提供状态获取、执行控制、教育功能和事件通知。</p>
 * 
 * <p>注意：该适配器需要RegisterVMInterpreter支持暂停点检查和事件触发机制。
 * 当前实现假设RegisterVMInterpreter已相应修改。</p>
 * 
 * @author TeachFX Team
 * @version 1.0
 * @since EP18R改进计划阶段一
 */
public class RegisterVMVisualAdapter implements IVirtualMachineVisualization, EventPublisher {
     ...
     * 反汇编器
     */
    private final RegisterDisAssembler disAssembler;
     ...
     * 虚拟机状态
     */
    private final VMState<RegisterVMSpecificState> vmState;
     ...
     * @param config VM配置
     */
    public RegisterVMVisualAdapter(RegisterVMInterpreter vm, VMConfig config) {
        if (vm == null) {
     ...
     * 创建反汇编器
     */
    private RegisterDisAssembler createDisAssembler(RegisterVMInterpreter vm) {
        try {
    ...
    private void initializeEventSystem() {
        ...
        eventBus.subscribe(new EventSubscriber<InstructionExecutedEvent>() {
            ...
            public String getSubscriberId() {
                return "RegisterVMVisualAdapter";
            }
    ...
    @Override
    public VMState<RegisterVMSpecificState> getCurrentState() {
        updateVMState();
        return vmState.copy();
    }
    ...
    public String disassembleInstruction(int pc) {
        if (disAssembler != null) {
            try {
                return disAssembler.disassembleInstructionToString(pc);
            } catch (Exception e) {
                // 回退到基本反汇编
            }
        }
    ...
    private void updateVMState() {
        ...
        // 更新特定状态
        RegisterVMSpecificState specificState = vmState.getVmSpecificState();
        specificState.updateRegisters(getRegisters());
        specificState.updateMemory(getMemorySnapshot());
     ...
     * 寄存器虚拟机特定状态
     */
    public static class RegisterVMSpecificState {
        private int[] registers;
        private int[] memory;
        private int callStackDepth;
        private Map<String, Object> additionalInfo;
        
        public RegisterVMSpecificState() {
        ...
        public void updateRegisters(int[] registers) {
            this.registers = registers != null ? Arrays.copyOf(registers, registers.length) : new int[0];
        }
        
        public void updateMemory(int[] memory) {
            this.memory = memory != null ? Arrays.copyOf(memory, memory.length) : new int[0];
        }
        ...
        public String toString() {
            return String.format(
                "RegisterVMSpecificState{registers=%d, memory=%d, callStackDepth=%d}",
                registers.length, memory.length, callStackDepth
            );
        }
    }
