...
│   ├── 52条统一指令 (opcode 0x00-0x6E)
│   └── EP18栈式VM + EP18R寄存器VM双格式支持
...
所有指令均为32位固定长度，采用统一的指令编码格式。EP18（栈式VM）和EP18R（寄存器VM）共享相同的底层格式，但使用不同的指令子集。
...
| v1.2 | 2025-12-23 | 32位定长指令集重构：更新指令格式为统一编码 |
