...
## 附录E：32位定长指令格式（v1.1新增）

### E.1 指令编码规范

EP18和EP18R虚拟机采用统一的32位定长指令格式，便于调试、验证和工具链集成。
...
所有指令使用**大端序**（Big-Endian）编码，即高位字节存储在低地址。
...
| 0x00 | nop | 控制 | 空操作 |
| 0x01 | halt | 控制 | 停止执行 |
| 0x02 | push | 栈操作 | 将立即数压栈（imm为立即数） |
| 0x03 | pop | 栈操作 | 弹出栈顶值 |
| 0x04 | dup | 栈操作 | 复制栈顶值 |
| 0x10 | iload | 内存 | 加载局部变量（imm为变量索引） |
| 0x11 | istore | 存储 | 存储局部变量（imm为变量索引） |
...
| 0x30 | icmp | 比较 | 整数比较 |
| 0x31 | zcmp | 比较 | 零比较 |
| 0x40 | beqz | 分支 | 等于零跳转（imm为偏移量） |
| 0x41 | bnez | 分支 | 不等于零跳转（imm为偏移量） |
| 0x42 | j | 控制 | 无条件跳转（imm为偏移量） |
| 0x43 | call | 函数 | 函数调用 |
| 0x44 | ret | 函数 | 函数返回 |
| 0x50 | print | I/O | 打印整数 |
...
| opcode | 助记符 | 描述 |
|--------|--------|------|
| 0x60 | mov rd, rs | 寄存器移动 |
| 0x61 | li rd, imm | 加载立即数到寄存器 |
| 0x62 | lw rd, offset(rs) | 加载字 |
| 0x63 | sw rs, offset(rd) | 存储字 |
| 0x64 | add rd, rs1, rs2 | 加法 |
| 0x65 | sub rd, rs1, rs2 | 减法 |
| 0x66 | mul rd, rs1, rs2 | 乘法 |
| 0x67 | div rd, rs1, rs2 | 除法 |
| 0x68 | slt rd, rs1, rs2 | 小于比较（set less than） |
| 0x69 | sle rd, rs1, rs2 | 小于等于比较 |
| 0x6A | sgt rd, rs1, rs2 | 大于比较 |
| 0x6B | sge rd, rs1, rs2 | 大于等于比较 |
| 0x6C | seq rd, rs1, rs2 | 相等比较（set equal） |
| 0x6D | sne rd, rs1, rs2 | 不等比较 |
| 0x6E | jf rs, offset | 条件跳转（假） |
...
```assembly
# 10 + 20 = 30
push 10       # 0x0200000A (push 10)
push 20       # 0x02000014 (push 20)
iadd          # 0x20000000
print         # 0x50000000
halt          # 0x01000000
```
