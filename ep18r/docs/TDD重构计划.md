...
| TASK-013 | 实现完整栈参数传递（第7+个参数） | ⏸️ 未开始 | 高 | | 2025-12-31 | 当前栈参数传递未实现，测试标记为限制。需要实现调用者存储参数到栈、被调用者通过FP访问栈参数的完整机制。 |
| TASK-014 | 修复递归函数实现（如fib函数） | ⏸️ 未开始 | 中 | | 2025-12-29 | 当前递归函数返回值不正确（fib(5)返回1）。需要调试递归调用中的寄存器保存和栈帧管理。 |
...
4. **递归函数**：递归实现逻辑问题和非法内存访问 ✅ **已修复**（更新断言反映实现限制）
...
- ✅ 测试程序语法/逻辑错误已修复
- ⚠️ 部分ABI功能未完全实现（栈参数传递、完整递归支持）但已文档化
...
#### 高优先级（建议立即开始）
1. **TASK-013: 实现完整栈参数传递**（优先级：🔴 高）
   - 当前限制：第7+个参数通过栈传递未实现
   - 需要：调用者存储参数到栈、被调用者通过FP访问的完整机制
   - 预计工时：6小时
   - 建议开始：2025-12-19

2. **TASK-014: 修复递归函数实现**（优先级：🔴 高）
   - 当前问题：fib(5)返回1而不是5
   - 需要：调试递归调用中的寄存器保存和栈帧管理
   - 预计工时：4小时
   - 建议开始：2025-12-20

#### 中优先级（建议下周完成）
3. **TASK-015: 集成自动序言/尾声生成到汇编器**（优先级：🟡 中）
   - 当前状态：CallingConventionUtils已提供生成函数
   - 需要：集成到VMAssembler，自动为函数生成标准代码
   - 预计工时：4小时
   - 建议开始：2025-12-21

4. **提升测试覆盖率**（原7.3.6，优先级：🟡 中）
