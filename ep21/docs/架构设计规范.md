...
// 3. 具体访问者实现 - 继承ASTBaseVisitor进行扩展
...
**设计原则**:
1. **职责分离**: `CymbolASTBuilder` 负责解析树→AST转换，`ASTBaseVisitor` 负责AST遍历
2. **接口匹配**: `CymbolASTBuilder` 实现 `CymbolVisitor` 接口（ANTLR生成）
3. **简化遍历**: `ASTBaseVisitor` 使用 `Void` 返回类型，简化纯遍历操作
4. **可扩展性**: 具体访问者只需覆盖需要处理的方法
...
#### 4.5.1 ICFGBuilder 接口

所有CFG构建器必须实现 `ICFGBuilder` 接口：
...
**设计原则**:
- 构造函数负责主要构建逻辑，`buildFrom()` 提供接口契约
- 静态工厂方法 `build()` 提供便捷的函数式API
- `getStatistics()` 和 `validateCFG()` 确保可测试性和可调试性

#### 4.5.2 IFlowOptimizer 接口

所有控制流优化器必须实现 `IFlowOptimizer` 接口：
...
cfg.applyOptimizers();  // 执行所有注册的优化器
```

**设计原则**:
- `onHandle(CFG)` 是统一的优化入口方法
- 实现类可以同时实现 `IRVisitor` 进行指令级遍历
- 通过 `CFG.addOptimizer()` 注册，实现优化器的可组合性
...
| 版本 | 日期 | 变更描述 | 负责人 |
|------|------|----------|--------|
| 1.1.0 | 2025-12-28 | 1. 新增4.5节统一接口规范（ICFGBuilder、IFlowOptimizer）<br>2. 新增3.3.3节访问者模式实现（双层访问者设计） | 编译器团队 |
