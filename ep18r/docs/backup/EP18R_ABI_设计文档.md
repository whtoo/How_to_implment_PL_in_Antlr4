...
| 项目 | 目标ABI规范 | 当前实现 | 说明 |
|------|-------------|----------|------|
| 返回值寄存器 | a0 (r2) | a0 (r2) | 已统一为r2（a0）以符合目标ABI规范 |
| 返回值寄存器ABI名称 | a0 | a0 | 对应的ABI名称已统一 |
| 工具类支持 | `CallingConventionUtils.getReturnValueRegister()` 应返回2 | 实际返回2 | 工具类已更新以反映统一实现 |
...
| 帧指针(fp) | r14作为帧指针 | 部分实现 | 帧指针支持正在开发中，内存访问指令已支持FP访问局部变量数组 |
| 局部变量存储 | 堆内存中的栈帧区域 | 独立的locals数组 | 局部变量仍存储在独立的locals数组中，尚未迁移到堆内存 |
...
| 局部变量访问 | 通过FP（帧指针）相对寻址 | 通过SP（栈指针）相对寻址，现已支持FP | 内存访问指令（LW/SW）已扩展支持FP作为基址寄存器访问局部变量数组 |
...
**迁移建议**：
1. 返回值处理：使用a0 (r2)作为返回值寄存器（已统一）
2. 参数传递：使用a0-a5 (r2-r7)传递参数（与规范一致）
3. 寄存器保存：CALL指令自动保存r2-r7，调用者无需手动保存
4. 栈帧访问：使用简化的栈帧模型，通过SP (r13)或FP (r14)访问局部变量（FP支持已添加）
...
3. 编写ABI一致性测试套件（第10节）
4. 实现真正的栈帧内存布局，将局部变量分配到堆内存中（当前仍使用独立的locals数组）
5. 逐步消除当前实现与ABI规范的差异

**已完成的ABI迁移步骤**:
- 统一返回值寄存器为a0 (r2)
- 扩展内存访问指令支持FP访问局部变量数组
- 更新CallingConventionUtils工具类以反映统一实现