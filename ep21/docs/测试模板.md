...
**目的**: 提供EP21 TDD重构的测试模板和最佳实践
**相关文档**: [TDD执行计划_精简版](TDD执行计划_精简版.md) | [详细任务分解](详细任务分解.md) | [研究生进阶任务](研究生进阶任务.md)
...
@DisplayName("AST节点测试：二元表达式")
class BinaryExprNodeTest {

    @Test
    @DisplayName("创建二元表达式节点应正确设置操作符和操作数")
...
@DisplayName("AST节点测试：函数声明")
class FuncDeclNodeTest {

    @Test
    @DisplayName("创建函数声明节点应正确设置属性")
...
@DisplayName("数据流分析测试：活跃变量")
class LiveVariableAnalyzerTest {

    @Test
    @DisplayName("简单赋值语句应正确识别活跃变量")
...
class SSAGraphTest {
    ...
    @Nested
    @DisplayName("SSA重命名测试")
    class SSARenamingTest {

        @Test
        @DisplayName("应正确重命名变量")
