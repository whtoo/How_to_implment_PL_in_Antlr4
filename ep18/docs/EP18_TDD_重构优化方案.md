...
graph TD
    ...
    TASK32 --> TASK42[TASK-4.2<br/>垃圾回收系统完善]
    ...
    ExceptionHandling --> Phase7[Phase7: 32位定长指令]
    Phase7 --> TASK71[TASK-7.1<br/>指令编码规范设计]
    Phase7 --> TASK72[TASK-7.2<br/>汇编器适配更新]
    Phase7 --> TASK73[TASK-7.3<br/>反汇编器适配更新]
    Phase7 --> TASK74[TASK-7.4<br/>EP21代码生成器适配]
    Phase7 --> TASK75[TASK-7.5<br/>验证测试]

    TASK71 --> FinalDelivery[最终交付]
    TASK72 --> FinalDelivery
    TASK73 --> FinalDelivery
    TASK74 --> FinalDelivery
    TASK75 --> FinalDelivery
...
| **阶段** | **Phase4** | **高级功能实现** | ⏸️ 未开始 | 中 | - | 2025-02-01 | 包含3个子任务（TASK-4.2含4个二级子任务） |
| 子任务 | TASK-4.1 | 调试器实现（VMDebugger、断点管理） | ⏸️ 未开始 | 中 | - | 2025-02-01 | 高级功能实现 |
| 子任务 | TASK-4.2 | 垃圾回收系统完善（GC集成与增强） | ⏸️ 未开始 | 中 | - | 2025-02-10 | 集成现有GC到虚拟机并增强功能 |
| 二级子任务 | TASK-4.2.1 | **GC引用计数契约修复** | 🔴 紧急 | 高 | Claude Code | 2025-12-23 | 修复GC测试失败问题：统一allocate/incrementRef/decrementRef契约 |
| 二级子任务 | TASK-4.2.1.1 | 修复allocate方法引用计数 | 🔴 紧急 | 高 | Claude Code | 2025-12-23 | allocate时不设置引用计数为1，由调用者决定是否持有引用 |
| 二级子任务 | TASK-4.2.1.2 | 重构线性分配器为空闲链表 | 🔴 紧急 | 高 | Claude Code | 2025-12-23 | 实现空闲链表管理，支持内存重用 |
| 二级子任务 | TASK-4.2.1.3 | 修复测试代码引用计数契约 | 🔴 紧急 | 高 | Claude Code | 2025-12-23 | 更新所有测试使用正确的引用计数模式 |
| 二级子任务 | TASK-4.2.2 | GC与虚拟机集成 | ⏸️ 未开始 | 高 | - | 2025-02-05 | 将ReferenceCountingGC集成到CymbolStackVM，替换简单堆管理 |
| 二级子任务 | TASK-4.2.3 | 循环引用检测增强 | ⏸️ 未开始 | 中 | - | 2025-02-07 | 改进引用计数GC的循环引用检测算法 |
| 二级子任务 | TASK-4.2.4 | GC性能基准测试 | ⏸️ 未开始 | 中 | - | 2025-02-08 | 建立GC性能基准和监控，测量暂停时间和吞吐量 |
| 二级子任务 | TASK-4.2.5 | GC系统文档和示例 | ⏸️ 未开始 | 低 | - | 2025-02-09 | 完善GC相关设计文档、API文档和使用示例 |
...
| **阶段** | **Phase6** | **VM适配支持** | ⏸️ 未开始 | 中 | EP21团队 | 2026-02-01 | 为EP21提供栈式VM代码生成支持，包含3个子任务 |
| 子任务 | TASK-18-VM-01 | 字节码定义接口抽象 BytecodeDefinitionInterface | ⏸️ 未开始 | 中 | EP21团队 | 2026-01-10 | 抽象字节码定义，便于EP21统一访问，提供统一的指令定义查询接口 |
| 子任务 | TASK-18-VM-02 | 栈帧布局导出 StackFrameLayoutExporter | ⏸️ 未开始 | 低 | EP21团队 | 2026-01-12 | 导出EP18栈帧布局信息供代码生成器使用，提供栈帧偏移量计算工具 |
| 子任务 | TASK-18-VM-03 | CymbolAssembler接口适配 | ⏸️ 未开始 | 中 | EP21团队 | 2026-01-15 | 适配CymbolAssembler为ICodeGenerator接口，支持EP21统一代码生成框架 |
| **阶段** | **Phase7** | **32位定长指令集重构** | 🔄 进行中 | 高 | Claude Code | 2025-12-30 | 统一EP18和EP18R指令编码为32位定长格式 |
| 子任务 | TASK-7.1 | 指令编码规范设计（32位定长格式定义） | 🔄 进行中 | 高 | Claude Code | 2025-12-25 | 定义 opcode(8) + rd(5) + rs1(5) + rs2(5) + imm(9) 格式 |
| 子任务 | TASK-7.2 | 汇编器适配更新（EP18和EP18R） | ⏸️ 未开始 | 高 | Claude Code | 2025-12-27 | 更新CymbolAssembler支持32位编码输出 |
| 子任务 | TASK-7.3 | 反汇编器适配更新 | ⏸️ 未开始 | 高 | Claude Code | 2025-12-28 | 更新反汇编器解析32位定长指令 |
| 子任务 | TASK-7.4 | EP21代码生成器适配（StackVMGenerator/RegisterVMGenerator） | ⏸️ 未开始 | 高 | Claude Code | 2025-12-29 | 更新代码生成器生成符合32位规范的字节码 |
| 子任务 | TASK-7.5 | 交叉验证测试（EP18与EP18R输出一致性） | ⏸️ 未开始 | 高 | Claude Code | 2025-12-30 | 确保两种VM输出可相互验证 |
...
|--------|------|----------|----------|------|--------|----------|------|
| ISSUE-001 | testStructOutOfMemory 测试失败：期望抛出 `OutOfMemoryError` 但未抛出 | `CymbolStackVMTest.testStructOutOfMemory:828` | 低 | ✅ 已解决 | Claude Code | 测试已更新：重构后结构体字段存储在Java堆中，不占用虚拟机堆空间，因此不再测试虚拟机堆内存不足场景 | 测试已通过，问题已关闭 |
| ISSUE-002 | 浮点指令未重构，与指令策略模式不兼容 | 算术指令优化任务遗留 | 低 | ✅ 已解决 | Claude Code | 已实现FAddInstruction、FSubInstruction、FMulInstruction、FDivInstruction、IToFInstruction等浮点指令策略类 | 浮点指令已重构完成，与指令策略模式兼容 |
| ISSUE-003 | 调试器功能缺失，影响开发体验 | 高级功能实现阶段 | 中 | ⏸️ 未开始 | - | 需要实现 VMDebugger 和断点管理 | Phase 4 任务 |
| ISSUE-004 | 垃圾回收系统未集成到虚拟机，内存管理效率待提升 | 高级功能实现阶段 | 中 | ⏸️ 未开始 | - | 需要将ReferenceCountingGC集成到CymbolStackVM并增强循环引用检测 | Phase 4 任务 |
...
#### 任务4.2：垃圾回收系统完善
- **目标**: 完善垃圾回收系统，将现有GC集成到虚拟机并增强功能
- **子任务**:
  1. GC与虚拟机集成：将ReferenceCountingGC集成到CymbolStackVM，替换简单堆管理
  2. 循环引用检测增强：改进引用计数GC的循环引用检测算法
  3. GC性能基准测试：建立GC性能基准和监控，测量暂停时间和吞吐量
  4. GC系统文档和示例：完善GC相关设计文档、API文档和使用示例
...
- **验收标准**:
  - ReferenceCountingGC成功集成到CymbolStackVM
  - 循环引用检测算法正确识别并处理循环引用
  - GC性能基准测试数据完整，暂停时间可测量
  - GC相关文档完整，包含API说明和使用示例
...
#### 垃圾回收系统完善
- **技术**: GC集成，循环引用检测，性能基准
- **目标**: 完善GC系统功能，提高内存管理效率
- **测量**: GC集成度，循环引用检测准确率，暂停时间
...
## 7.6 32位定长指令集重构（Phase 7）

### 7.6.1 重构背景与目标

**背景**：
当前EP18和EP18R使用不同格式的字节码表示，EP18为栈式VM（.vm），EP18R为寄存器VM（.vmr）。为简化调试、验证和工具链统一，需要将指令编码统一为32位定长格式。

**目标**：
- 统一EP18和EP18R的指令编码格式
- 使用32位定长指令，便于十六进制调试和验证
- 保持与现有汇编/反汇编器的兼容性
- 支持EP21代码生成器（StackVMGenerator/RegisterVMGenerator）输出合规字节码
...
| 字段 | 位宽 | 描述 |
|------|------|------|
| opcode | 8位 | 操作码，定义指令类型（0-255） |
| rd | 5位 | 目标寄存器（0-31），EP18R使用 |
| rs1 | 5位 | 源寄存器1（0-31），EP18R使用 |
| rs2 | 5位 | 源寄存器2（0-31），EP18R使用 |
| imm | 9位 | 立即数或偏移量（-256 ~ 255） |
...
| 0x00 | nop | 控制 | 空操作 |
| 0x01 | halt | 控制 | 停止执行 |
| 0x02 | push | 栈操作 | 将立即数压栈 |
| 0x03 | pop | 栈操作 | 弹出栈顶值 |
| 0x04 | dup | 栈操作 | 复制栈顶值 |
| 0x10 | iload | 内存 | 加载局部变量 |
| 0x11 | istore | 存储 | 存储局部变量 |
| 0x12 | gload | 内存 | 加载全局变量 |
| 0x13 | gstore | 存储 | 存储全局变量 |
| 0x20 | iadd | 算术 | 整数加法 |
| 0x21 | isub | 算术 | 整数减法 |
| 0x22 | imul | 算术 | 整数乘法 |
| 0x23 | idiv | 算术 | 整数除法 |
...
### 7.6.3 TASK-7.1: 指令编码规范设计

**目标**
定义完整的32位定长指令编码规范，包括所有指令的操作码分配和二进制格式。
...
### 7.6.4 TASK-7.2: 汇编器适配更新

**目标**
更新CymbolAssembler支持输出32位定长格式的字节码文件（.vm和.vmr）。
...
### 7.6.5 TASK-7.3: 反汇编器适配更新

**目标**
更新反汇编器能够正确解析32位定长指令格式。
...
### 7.6.6 TASK-7.4: EP21代码生成器适配

**目标**
更新StackVMGenerator和RegisterVMGenerator生成符合32位规范的字节码。

**子任务**
1. 更新StackVMGenerator输出格式
2. 更新RegisterVMGenerator输出格式
3. 统一两个生成器的输出规范
4. 验证交叉VM输出的一致性
...
### 7.6.7 TASK-7.5: 交叉验证测试

**目标**
确保EP18和EP18R在相同输入下产生可验证的输出。
...
### 里程碑6：32位定长指令集重构（第17周结束）
- [ ] 指令编码规范设计完成（TASK-7.1）
- [ ] 汇编器适配更新完成（TASK-7.2）
- [ ] 反汇编器适配更新完成（TASK-7.3）
- [ ] EP21代码生成器适配完成（TASK-7.4）
- [ ] 交叉验证测试通过（TASK-7.5）
...
6. 更新相关文档和示例
7. 建立异常处理最佳实践指南
8. **启动32位定长指令集重构（Phase 7）**
9. 定义指令编码规范和操作码分配
10. 更新汇编器/反汇编器支持32位格式
11. 适配EP21代码生成器（StackVMGenerator/RegisterVMGenerator）
12. 编写交叉验证测试确保EP18与EP18R输出一致

**成功宣言**:
> 通过系统的TDD重构，我们将把EP18堆栈式虚拟机打造成为教育领域的标杆项目，为学习编译器构建和虚拟机实现提供最佳实践。
>
> **v3.0新增**: 32位定长指令集重构将使EP18和EP18R的字节码格式统一，便于调试、验证和工具链集成，提升开发体验和代码质量。