...
| 项目 | 目标ABI规范 | 当前实现 | 说明 |
|------|-------------|----------|------|
| 栈帧结构 | 标准栈帧布局 | 简化栈帧（`StackFrame`类） | 当前实现使用简化的栈帧结构 |
| 栈帧对齐 | 8字节对齐 | 未强制对齐 | 对齐要求尚未实现 |
| 帧指针(fp) | r14作为帧指针 | 部分实现 | 帧指针支持正在开发中 |
| 局部变量存储 | 堆内存中的栈帧区域 | 独立的locals数组 | 局部变量仍存储在独立的locals数组中 |
| 内存地址计算 | 字节寻址 | 字寻址（int数组） | 堆内存使用int数组，字节偏移需要除以4 |
| 被调用者保存寄存器 | s0-s4完整支持 | 部分支持 | 完整的被调用者保存寄存器支持正在开发中 |

**迁移建议**：
1. 使用a0 (r2)作为返回值寄存器（已统一）
2. 使用a0-a5 (r2-r7)传递参数（与规范一致）
3. CALL指令自动保存r2-r7，调用者无需手动保存
4. 使用SP (r13)或FP (r14)访问局部变量（FP支持已添加）
